...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, PngError> {
        let remove_chunk_pos = self
            .chunks()
            .iter()
            .position(|c| c.chunk_type().to_string() == chunk_type);

        match remove_chunk_pos {
            Some(index) => Ok(self.chunks.remove(index)),
            None => Err(PngError::ChunkNotFound(chunk_type.to_string())),
        }
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err("Header is not the same!");
        }

        let mut chunks = Vec::<Chunk>::new();
        loop {
            let length_bytes = <[u8; 4]>::try_from(&chunk_bytes[..4]).unwrap();
            let length = u32::from_be_bytes(length_bytes) as usize;

            let chunk_total_size = length + 12;
            let (curr_chunk, rem_chunks) = chunk_bytes.split_at(chunk_total_size);

            let chunk = Chunk::try_from(curr_chunk).unwrap();
            chunks.push(chunk);
            chunk_bytes = rem_chunks;
            if rem_chunks.len() == 0 {
                break;
            }
        }
...
impl fmt::Display for PngError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ChunkNotFound(ct) => {
                write!(f, "Couldn't find chunk with {} chunk type.", ct)
            }
        }
    }
}
...
mod tests {
    ...
    fn chunk_from_strings(chunk_type: &str, data: &str) -> types::Result<Chunk> {
